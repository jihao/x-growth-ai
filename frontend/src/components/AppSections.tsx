...
import { api } from "../api";
import { REVIEW_STORAGE_KEY, technicalKeys, type Page, type ReviewNote, type TechnicalKey } from "../appTypes";
import type {
  BacktestJob,
  Candidate,
...
function PanelTitle({ icon: Icon, title }: { icon: typeof Home; title: string }) {
...
function reviewCell(value: unknown): string {
...
function formatIndexPoint(value: number | null): string {
...
function signedlessPct(value: number | null): string {
  if (value === null) return "-";
...
function ratioText(up: number | undefined, down: number | undefined): string {
  if (!up || !down) return "-";
  return `${(up / down).toFixed(2)}:1`;
}

function breadthLabel(overview: MarketOverview | undefined): string {
...
function ppText(value: number | null | undefined): string {
